...
float x=22.0f, y=10.0f, z=-20.0f; // posisi awal kamera
float lx=0.0f, ly=0.0f, lz=-1.0f;
float tx=0.0, ty=0.0, tz=-10.0; //posisi truk
...
//float posYTruk=6, posZTruk=10;
//float rotTruk = 0.0;
//static int rotAngleX =0, rotAngleY =0;
...
//gameplay mechanic and renders is done here
void display() {
	glClearColor(0.64, 0.79, 0.99, 0);
    if (!debugCamera){
        //pergerakan truk
        if (speedX){
            moveTruk(putaranTruk, speedX*3);
        }
        if (!gasDitekan){
            if (speedX>0.1){
    ...
    Jalan();
    TrukGandeng(putaranTruk, tx, ty, tz);
